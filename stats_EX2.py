...
#scores = np.load('results_ex2G.npy')
scores = np.load('results_ex2M.npy')

headers = list(clfs.keys())
names_column = np.expand_dims(np.array(datasets), axis=1)
print(len(names_column),len(names_column[0]),names_column)

mean_scores = np.mean(scores, axis=2).T
print(len(mean_scores),len(mean_scores[0]),mean_scores)
